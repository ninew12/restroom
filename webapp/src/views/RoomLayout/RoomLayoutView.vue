...
export default {
  ...
  methods: {
    ...
    async submitForm() {
      ...
      await axios
        .post(`http://localhost:3899/buildings`, body, {
          headers: {
            // remove headers
    ...
    async submitRoom() {
      ...
      // await floorsLoop.forEach((x) => {
        axios.post(`http://localhost:3899/rooms`, floorsLoop, {
          headers: {
            "Access-Control-Allow-Origin": "*",
            "Content-Type": "application/json",
          },
    ...
    async getBuildingByid(id) {
      try {
        await axios
          .get(`http://localhost:3899/buildings/${id}`)
          .then((res) => {
            let data = res.data;
    ...
    submitDelete() {
      axios
        .delete(`http://localhost:3899/buildings/${this.builId}`)
        .then((res) => {
          setTimeout(() => {
            this.getAlluser();
          }, 1000);
